...
function currentRaisingFloor(): { label: string; multiplier: number } {
...
function getRaisingDueMs(item: RaisingItem, pig: Pig): number {
  return item.lastFedAt + adjustedFeedIntervalMs(pig);
}

function getRaisingClockNow(item: RaisingItem, now = Date.now()): number {
  const pausedAt = Number(item && item.pausedAt) || 0;
  return pausedAt > 0 ? pausedAt : now;
}

function getRaisingRemainingMs(item: RaisingItem, pig: Pig, now = Date.now()): number {
...
interface RaisingCloudRecord {
  id: string; pNo: number; pigName: string; floor: RaisingFloor;
...
function raisingStatusClass(dueMs: number): string {
...
function markRaisingFed(id: string): void {
...
function adjustRaisingFeedCount(id: string, delta: number): void {
  const item = state.raisingPigs.find(x => x.id === id);
...
function toggleRaisingPause(id: string): void {
  const item = state.raisingPigs.find(x => x.id === id);
  if (!item || item.status === "waiting") return;
  const pig = getPigByPNo(item.pNo);
  const now = Date.now();
...
function moveRaisingPig(id: string): void {
  const item = state.raisingPigs.find(x => x.id === id);
  if (!item) return;
  const pig = getPigByPNo(item.pNo);
...
async function removeRaisingPig(id: string): Promise<void> {
  const item = state.raisingPigs.find(x => x.id === id);
...
async function clearRaisingPigs(): Promise<void> {
...
function searchRaisingPigs(q: string): Pig[] {
...
function pushSubscribeErrorToast(err: unknown): void {
  const classified = err instanceof Error && (err as Error & { type?: string }).type ? err as Error & { type: string; canRetry?: boolean } : classifyPushError(err);
  const suffix = classified.canRetry ? ",可刷新页面后重试" : "";
  toast(`${classified.message}${suffix}`, 4000);
}

async function requestRaisingNotificationPermission(): Promise<void> {
  if (!notificationsSupported()) {
    toast("当前浏览器不支持系统通知");
    updateRaisingNotificationButton();
    return;
