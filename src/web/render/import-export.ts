...
import type { Pig, ParsedImport, ImportApplyResult, ExportPayload, RaisingItem, RaisingFloor } from "../js/types.js";
...
function searchByName(q: string): Pig[] {
...
// ---- 三元组添加 ----
function parseTriple(book: string, page: string, slot: string): { err?: string; book?: number; page?: number; slot?: number; listno?: number } {
...
function addByPNo(pNo: number): { ok?: boolean; err?: string; msg?: string; pig?: Pig } {
...
function addFromTriple(book: string, page: string, slot: string): { ok?: boolean; err?: string; msg?: string; pig?: Pig } {
  if (!state.dataLoaded) return { err: "数据还没加载好" };
...
// ---- 批量三元组 ----
function parseBatchLines(text: string): Array<{ raw: string; idx: number; parts: string[] }> {
  const lines = text.split(/\r?\n/);
  const parsed: Array<{ raw: string; idx: number; parts: string[] }> = [];
...
// ---- 导入 ----
function parseImportText(raw: string): ParsedImport {
  const txt = (raw || "").trim();
  const emptyResult = (err: string): ParsedImport => ({ err, collection: [], ownedEventPigs: [], smallBadges: [], bigBadges: [], raisingPigs: [], source: "json", formatVersion: 0 });
...
function applyImport(parsed: ParsedImport, { replace }: { replace: boolean }): ImportApplyResult {
