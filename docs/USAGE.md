<p align="center">
  <img src="../static/icon-512.png" alt="猪猪图鉴" width="128">
</p>

<h1 align="center">猪猪图鉴 · 使用手册</h1>

<p align="center">
  一个跑在浏览器里的小 PWA，帮你整理《猪猪图鉴》186 只图鉴的收藏进度。<br>
...
│                                      │
├──────────────────────────────────────┤
│  186图鉴  Events图鉴  养成中  拍卖场  我的 │ ← 底部 tab 栏
└──────────────────────────────────────┘
```
...
- **它能配出的崽**：反向列出「这只猪作为父本之一」时的所有配方；对 **活动猪** 产出会在概率 chip 旁边附一个 `☐ 未拥有 / ☑ 已拥有` 勾选，单独存在 `localStorage` 的 `pig_owned_event_v1` 里，不影响 186 主猪的收藏
- **从收藏移除** 按钮（红色，底部）
...
「养成中」用于记录正在养的猪。顶部搜索框会在全部图鉴猪里搜索，点击结果即可加入养成列表；同一种猪可以重复加入，用来对应游戏里同时养多只的情况。
...
每条养成记录会显示上次喂食、下次喂食、已喂食次数和倒计时。点 **「已喂食」** 会重置这只猪的倒计时，并把喂食次数 +1；也可以用记录里的 `+ / -` 手动调整。达到最少喂食次数后会显示达标状态。点 **「开启提醒」** 后，到点时会尝试发送系统通知。

需要暂时停止计时时，点倒计时上方的 **「💊 晚安药」**。剩余时间和进度会冻结，本地提醒与后台推送也会暂停；点 **「☀️ 唤醒」** 后，会从冻结的位置接着计时。

> 提醒由浏览器/PWA 前端计时触发。页面或已安装 PWA 存活时可用；如果应用被系统完全关闭，浏览器不会继续跑倒计时。要关闭后仍准时提醒，需要后端 Web Push/定时任务。
...
「我的」tab 里的 **导出 / 导入配置** 区，负责备份和恢复——一次性把 **收藏**、**活动猪「已拥有」勾选**、**大小章**、**养成中记录** 和 **地板设置** 一起带走。

**导出**

- 点 **「导出」** → 文本框里出现一段 JSON（形如 `{ "type": "pigfarm-helper-backup", "owned186Pigs": [...], "ownedEventPigs": [...], "raisingPigs": [...] }`）
...
- **覆盖导入**：弹原生 `confirm` 确认后，用导入的数据 **完全替换** 当前收藏、「已拥有」勾选、大小章和养成记录（⚠️ 不可恢复）
- **清空输入**：清掉文本框

> 🧰 **兼容旧格式**：如果粘贴的不是 JSON 而是老版「图鉴/页/格」三元组列表（每行一条，`#` 开头是注释），也能识别，但只会恢复收藏部分、不会恢复「已拥有」勾选、大小章或养成记录。

### 清空收藏

最底部 **记录管理** 区的「清空记录」按钮 —— 点击会弹原生 `confirm` 确认框，确认后清空当前所有收藏、活动猪「已拥有」勾选、大小章和养成记录。⚠️ 无法恢复。
...
**Q：收藏能在多个设备间同步吗？**
A：不能。纯本地存储，不走任何后端。搬家推荐用「我的」tab 里的 [导出 / 导入配置](#导出--导入配置)——一键备份收藏、活动猪「已拥有」勾选、大小章和养成记录，跨设备恢复。

**Q：离线用能翻全部图鉴信息吗？**
A：能。首次联网加载后 Service Worker 会把 `pigs_full.json` 和 UI 壳子缓存下来；远端的猪图也会按需缓存。完全断网也能翻你已经加载过的任意猪的详情。
