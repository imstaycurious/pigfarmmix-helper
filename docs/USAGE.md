...
> 💾 **数据只存在本机**。换设备/浏览器/清缓存 → 收藏会丢。如果在意，先在浏览器「导出 localStorage」备份。
...
- **获得方式**：商店各等级概率 / 狩猎场所和券种 / 任务要求 / 养成失败可得自…
...
最底部 **收藏管理** 区的「清空收藏」按钮 —— 点击会弹原生 `confirm` 确认框，确认后清空当前所有收藏。⚠️ 无法恢复。
...
**Q：收藏能在多个设备间同步吗？**
A：不能。纯本地存储，不走任何后端。要搬家可以导出/导入 localStorage 的 `pig_collection_v1` 这个 key（键值是 pNo 数组的 JSON）。
