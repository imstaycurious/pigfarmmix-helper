...
function filterAuctionByOwn(records, own) {
    ...
    return records.filter(rec => {
        ...
        const owned = known && (isEventPigId(pNo)
        ...
        if (own === "yes")
            return owned;
        if (own === "no_small")
            return owned && !state.smallBadges.has(pNo);
        if (own === "no_big")
            return owned && !state.bigBadges.has(pNo);
        return true;
    });
}
...
function buildAuctionOwnershipRow(pNo) {
    ...
    return el("div", { class: "auction-own-row" }, [
        el("span", { class: "auction-own-chip pig" + (owned ? " is-on" : "") }, owned ? "✅ 已拥有" : "⬜ 未拥有"),
        el("img", {
            src: "/img/small.png",
...
function buildAuctionRow(rec) {
    ...
    const sublineParts = [];
    if (pig && pig.color_text)
        sublineParts.push(pig.color_text);
    else
        sublineParts.push(`bType=${rec.bType}`);
    ...
    if (ownRow && forecast)
        ownRow.appendChild(forecast);
    const info = el("div", { class: "info" }, [
        el("div", { class: "name" }, nameChildren),
        el("div", { class: "meta" }, metaParts.join(" · ")),
        el("div", { class: "owner", title: rec.ownername }, `${sublineParts.join(" · ")} · 出品 ${rec.ownername || "(匿名)"} · #${rec.pigNo}`),
        ownRow,
    ]);
    ...
    return el("div", {
        ...
        onclick: () => {
            if (lookupPig(rec.bType))
                runtime.showDetail(rec.bType);
        },
    }, [thumb, info, priceCol]);
}
...
async function fetchAuctions({ append = false, server } = {}) {
    if (auctionState.loading || auctionState.loadingMore)
        return;
    if (!append) {
        ...
        if (server)
            auctionState.server = server;
    }
    if (append)
        auctionState.loadingMore = true;
    else
...
async function loadMoreAuctions() {
    if (auctionState.loading || auctionState.loadingMore || auctionState.atEnd)
        return;
    auctionState.count += AUCTION_PAGE_SIZE;
    await fetchAuctions({ append: true });
}
export function setupAuction() {
    // 筛选 chip 点击
...
export function renderAuctionTabEntry() {
    renderAuctionTab();
}
export function stopAuctionForTeardown() {
    stopAuctionCountdown();
    if (auctionLoadMoreObserver) {
        auctionLoadMoreObserver.disconnect();
        auctionLoadMoreObserver = null;
