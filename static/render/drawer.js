...
export function showDetail(pNo) {
    const p = state.pigsById.get(pNo) || state.eventPigsById.get(pNo);
    if (!p)
        return;
    ...
    for (const g of acqOrder) {
        ...
        acqHTML.push(`<div class="kv"><div class="k">${METHOD_LABELS[g] || g}</div>` +
    ...
    const renderOutcomeSlot = (k, prob, _opts) => {
        const pNo = k.pNo;
    ...
    for (const b of asParent) {
        const k = String(b.isview);
        if (!asParentByView.has(k))
            asParentByView.set(k, []);
  ...
  `;
    ...
    if (cbtn) {
        cbtn.addEventListener("click", async () => {
            const wasOwn = isOwn;
            if (!(await setPigOwnedAfterConfirm(p.pNo, !wasOwn)))
                return;
    ...
    if (rbtn)
        rbtn.addEventListener("click", () => runtime.addRaisingPig(p.pNo));
    const wbtn = document.getElementById("drawerWaitingBtn");
    if (wbtn)
        wbtn.addEventListener("click", () => runtime.addRaisingPig(p.pNo, "waiting"));
...
export function getCurrentDetailPNo() {
    return currentDetailPNo;
}
let toast = () => { };
let updateOwnedUIImpl = () => { };
export function setDrawerDeps(deps) {
    toast = deps.toast;
...
export function setupDrawer() {
    ...
    drawer.addEventListener("pointerdown", (e) => {
        ...
        if (!onHandle && drawer.scrollTop > 0)
            return;
        armed = true;
        dragging = false;
        startY = e.clientY;
    ...
    drawer.addEventListener("pointermove", (e) => {
        if (!armed || e.pointerId !== activePointerId)
            return;
        currentY = e.clientY;
        ...
        if (dy > DRAG_START_PX) {
            ...
            if (drawer.setPointerCapture && activePointerId !== null) {
                try {
                    drawer.setPointerCapture(activePointerId);
                }
                catch { /* ignore */ }
            }
        }
    ...
    content?.addEventListener("click", async (e) => {
        ...
        if (chk) {
            ...
            if (!pNo)
                return;
            if (!(await setPigOwnedAfterConfirm(pNo, !state.ownedEventPigs.has(pNo))))
                return;
            updateOwnedUIImpl(pNo);
            if (currentDetailPNo)
                showDetail(currentDetailPNo);
        ...
        if (!target || target === currentDetailPNo)
            return;
        e.stopPropagation();
        showDetail(target);
        if (content)
