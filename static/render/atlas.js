...
function buildProgressBuckets() {
    const main = { byRare: new Map(), byColor: new Map(), byBadge: new Map() };
    ...
    const event = { byRare: new Map(), byColor: new Map(), byBadge: new Map() };
...
export function renderProgressPanel() {
    ...
    for (const p of state.eventPigsById.values()) {
        if (p.rare === 1)
            continue;
        eventTotal++;
        if (state.ownedEventPigs.has(p.pNo))
            eventOwned++;
    }
...
function renderEmpty(box, title, hint) {
    box.appendChild(el("div", { class: "empty" }, [
        el("div", { class: "title" }, title),
        el("div", { class: "hint" }, hint),
    ]));
}
export function renderAtlasBody() {
    const box = $("#atlasBody");
    if (!box)
        return;
    box.innerHTML = "";
    ...
    for (const p of pigs)
        grid.appendChild(buildCard(p, { showCollected: false, showBadges: false }));
    box.appendChild(grid);
}
export function renderEventsBody() {
    const box = $("#eventBody");
    if (!box)
        return;
    box.innerHTML = "";
...
export function renderMineBody() {
    renderMineMenuCounts();
    renderProgressPanel();
    if (state.mineView !== "main" && state.mineView !== "event")
        return;
    const box = $("#mineBody");
    if (!box)
        return;
    ...
    const grid = el("div", { class: "grid" });
    for (const p of pigs)
        grid.appendChild(buildCard(p, { showCollected: true, showBadges: true }));
    box.appendChild(grid);
}
// ----- 统计条 -----
export function renderAtlasStats() {
...
export function renderEventsStats() {
...
export function renderMineStats() {
    ...
    if (!state.dataLoaded || (state.mineView !== "main" && state.mineView !== "event")) {
        ...
        let sm = 0, bg = 0;
        for (const pNo of state.smallBadges)
            if (inScope(pNo))
                sm++;
        for (const pNo of state.bigBadges)
            if (inScope(pNo))
                bg++;
...
// 定点刷新卡片
export function refreshPigCards(pNo) {
