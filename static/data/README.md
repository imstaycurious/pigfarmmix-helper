...
**主要改进：**
- 文件大小减少 65.3% (1.8MB → 0.63MB)
- 配种关系独立存储，去除冗余嵌套对象
- 用 `status` 字段替代硬编码的隐藏/移除猪列表
- 统一养成要求字段 (`breed_note` + `breeding_guide` → `breedingGuide`)
- 字段重命名为驼峰命名和语义化

**技术细节：**
- 配种关系从 645 条记录去重到 626 条
- 嵌套对象从 1985 个减少到 0 个 (改为 pNo 引用)
- `arrival_bleed` 中的完整猪对象 → 独立的 `breeding` 表

**字段映射：**
- `list.typeno/listno` → `atlas.type/index`
- `bigWeight/smallWeight` → `weight.big/small`
- `isExer` → `graze`
- `eatable/eatable_time/eat_times` → `feeding.picky/interval/times`
- `add_rank` → `acquisition.shop`
- `arrival_place/hunt_prob` → `acquisition.hunt.sites/prob`
- `arrival_fail` → `acquisition.fail` (只存 pNo)
- `feed_special` → `acquisition.specialFeeding` (布尔值)
- `breed_note` + `breeding_guide` → `breedingGuide`
- `arrival_bleed` → 独立的 `breeding` 表

### v2 (之前)

- 原始数据结构
- 配种关系嵌套在每只猪的 `arrival_bleed` 字段中
- 包含大量冗余的完整猪对象
...
数据来源于 [养猪场MIX 攻略Wiki](https://pigfarmmix.net/)

---

## 许可证

数据仅供学习和个人使用，请勿用于商业用途。
