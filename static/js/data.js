...
async function cacheBundle(bundle) {
    try {
...
async function loadFromApi() {
    try {
        ...
        if (!data || !Array.isArray(data.pigs))
            return null;
        return data;
    }
    catch {
...
export function enrichPig(p) {
    ...
    if (typeof p.graze === "boolean") {
        ...
        p.isExer = p.isExer === true || p.isExer === 1;
        p.graze = p.isExer;
    }
    return p;
}
...
export function buildBreedingIndex(breedingTable) {
    ...
    function addEntry(pNo, entry) {
        ...
        if (!seen.has(pNo))
            seen.set(pNo, new Set());
        const s = seen.get(pNo);
        if (s.has(key))
            return;
        s.add(key);
        if (!state.breedByParent.has(pNo))
            state.breedByParent.set(pNo, []);
...
export function pigMatchesHunt(pig, region, ticket) {
    ...
    for (const code of sites) {
        ...
        if (region) {
            ...
            if (!pair || pair.indexOf(code) < 0)
                continue;
        }
        if (ticket === "normal" && !HUNT_NORMAL_CODES.has(code))
            continue;
        if (ticket === "rare" && !HUNT_RARE_CODES.has(code))
            continue;
        return true;
    }
    return false;
...
export function setPigOwned(pNo, owned) {
    ...
    if (isEvent) {
        if (owned)
            state.ownedEventPigs.add(pNo);
        else
            state.ownedEventPigs.delete(pNo);
