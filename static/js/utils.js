...
export function badgeMetaHTML(pig) {
    ...
    const chip = (kind, ownedAttr, value, op, iconSrc, label) => `<div class="badge-chip badge-${kind}${ownedAttr ? " is-on" : ""}"` +
...
export function feedIntervalText(eatable_time) {
    if (eatable_time == null)
        return "?";
    if (eatable_time === 0)
        return "58 分钟";
    if (eatable_time < 1)
        return `${Math.round(eatable_time * 60)} 分钟`;
...
export function pigPicky(p) {
    ...
    if (ids.length === 0)
        return { level: "none", label: "不挑食", foods };
    if (ids.length === 1)
        return { level: "picky", label: "挑食", foods };
...
export function pigIsOwned(p) {
    if (p.book === 7)
        return state.ownedEventPigs.has(p.pNo);
...
export function showUnlockCelebration() {
  ...
  `;
    ...
    modal.addEventListener("click", e => {
        if (e.target === modal || e.target.id === "celebrationOk")
            close();
    });
}
