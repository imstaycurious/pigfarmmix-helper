...
export function filterPigs(pigs, filter) {
    ...
    return pigs.filter(p => {
        if (color && p.color_text !== color)
            return false;
        if (rare && String(p.rare) !== rare)
            return false;
        if (method && !pigHasMethod(p, method))
            return false;
        if (method === "hunt" && !pigMatchesHunt(p, huntRegion || "", huntTicket || ""))
            return false;
        if (method === "shop" && !pigMatchesShopRank(p, shopRank || ""))
            return false;
        if (graze === "yes" && !p.isExer)
            return false;
        if (graze === "no" && p.isExer)
            return false;
        if (picky && pigPicky(p).level !== picky)
            return false;
        if (ql) {
...
export function sortPigs(pigs) {
    return pigs.slice().sort((a, b) => (a.book - b.book) || (a.page - b.page) || (a.slot - b.slot) || (a.pNo - b.pNo));
}
// 活动猪筛选
...
export function currentMinePigs() {
    ...
    const out = [];
    const sources = [];
    if (state.mineView === "main")
        sources.push(state.pigsById.values());
    else if (state.mineView === "event")
        sources.push(state.eventPigsById.values());
    for (const iter of sources) {
        for (const p of iter) {
            ...
            if (small === "yes" && !hasSmall)
                continue;
            if (small === "no" && hasSmall)
                continue;
            if (big === "yes" && !hasBig)
                continue;
            if (big === "no" && hasBig)
