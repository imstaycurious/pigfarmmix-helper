...
import { loadData, checkAndUnlockHidden } from "./js/data.js";
...
import { renderAtlasBody, renderEventsBody, renderMineBody, renderAtlasStats, renderEventsStats, renderMineStats, renderMineMenuCounts, renderProgressPanel, refreshPigCards, } from "./render/atlas.js";
import { showDetail, closeDrawer, setDrawerDeps, setupDrawer } from "./render/drawer.js";
import { setUpdateOwnedUI } from "./render/cards.js";
import { addRaisingPig, renderRaisingBody, renderRaisingSearchResults, updateRaisingCountdownNodes, checkRaisingReminders, saveRaisingState, syncRaisingFloorSelect, startRaisingTicker, setupRaising, makeRaisingId, } from "./render/raising.js";
...
function resetChipRow(rootSel) {
    $$(".chip", $(rootSel) || document.body).forEach(c => c.classList.toggle("active", c.dataset.value === ""));
}
...
function makeMethodSubUpdater(prefix, filterObj) {
    ...
    return function update() {
        ...
        const s = $(shopSel);
        if (r)
            r.style.display = showHunt ? "" : "none";
        if (t)
            t.style.display = showHunt ? "" : "none";
        if (s)
            s.style.display = showShop ? "" : "none";
        if (!showHunt) {
            filterObj.huntRegion = "";
            filterObj.huntTicket = "";
            resetChipRow(regionSel);
            resetChipRow(ticketSel);
        }
        if (!showShop) {
            filterObj.shopRank = "";
...
function setMineView(view) {
    ...
    if (subheadTitle)
        subheadTitle.textContent = MINE_VIEW_TITLES[view] || "";
    // 星级筛选显示调整
    const mineRareFilter = $("#mineRareFilter");
    if (mineRareFilter && (view === "main" || view === "event")) {
        mineRareFilter.querySelectorAll(".chip").forEach(chip => {
...
function updateGlobalCounts() {
    ...
    if (mc)
        mc.textContent = `186 已拥有 ${state.collection.length} 只 · Events 已拥有 ${state.ownedEventPigs.size} 只 · 小章 ${state.smallBadges.size} · 大章 ${state.bigBadges.size}`;
}
...
async function clearAllRecords() {
    ...
    if (!(await customConfirm("确定要清空全部记录吗?")))
        return;
    state.collection = [];
...
function init() {
    ...
    if ("serviceWorker" in navigator) {
        navigator.serviceWorker.addEventListener("message", (e) => {
            if (e.data && e.data.type === "open-tab")
    ...
    if (/iPad|iPhone|iPod/.test(ua) && !window.navigator.standalone) {
        ...
        const t = $("#installText");
        if (t)
            t.textContent = "在 Safari 点击分享 → 加到主屏幕";
    }
    // 账号 UI
